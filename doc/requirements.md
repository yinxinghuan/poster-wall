...
- 无头像演出海报 prompt：用户名成为主视觉身份来源，转成大面积裁切字母、首字母、票据编码、场馆戳、手写 stage name、vertical fragments 或半可读标题，而不是普通署名。无头像路径会额外传入 `public/img/style-ref/flat-poster-ref.png` 作为格式参考图，只约束“满版平面海报文件 + 标题 / 日期 / 场地 / 编号的文字层级”这一形式，不作为墙面打底图，也不复制其具体文字、颜色和布局。UI 提示“Create an avatar to unlock a more personal poster identity.”
- 正式墙面只显示真实生成并保存的玩家海报；没有真实作品时显示空墙状态，不用默认海报填充。评审页可展示 `public/img/review-generated/*.jpg` 作为生成方向样张，但这些样张不参与正式墙面数据。
- 资产清单：`public/img/style-ref/flat-poster-ref.png` 是无头像生成的纯平面排版格式参考，包含通用演出文字层级但不包含品牌或禁用提示词；`public/img/review-generated/*.jpg` 是评审页生成方向样张；`public/poster.png` 是 1:1 平台封面；`public/img/aigram.svg` 是水印。
