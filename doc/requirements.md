...
- 顶部信息区高度约 104px，左右各 22px。左侧为 154px × 82px 黄色切角导视牌，clip-path 为右上和左下切角，显示 9px `POSTER WALL`、36px Impact/Arial Black 标题 `TONIGHT`、10px `WALL / MINE {n}`；右侧为 182px × 82px 的并排模式切换牌，两个按钮分别是 `STACK / Overlap` 与 `GRID / Order`。`STACK` 按钮内置 3 张重叠矩形图标，`GRID` 按钮内置 2×2 网格图标；选中态上移 5px，`STACK` 选中为红底黄字，`GRID` 选中为蓝底奶白字。
- 列表页核心是同一批海报在“叠放 / 排列”两种布局之间动画切换。首屏渲染最近 10 张作品，数据仍保留公共墙最多 24 张。叠放模式下最新海报尺寸 214px × 321px，在墙面区域内位置 x=88px、y=54px，旋转 -2deg，z-index 40；其余海报用 150px × 225px 到 190px × 285px 的尺寸从左右后方露出，旋转 -13deg 到 12deg，形成凌乱但有控制的重叠。排列模式下同一批元素在 680ms 内移动到 2 列整齐队列，每张 118px × 177px，旋转归零，列间距 26px，行距 14px，可纵向滚动。
...
- 底部 118px 是简洁操作区。入口左右各 22px、底部 18px，头像状态使用 34px 圆形头像和两行英文 10px 文案。主 CTA 高 54px，黄色切角标牌背景 `#ffe100`，主文案为 27px Impact `MAKE POSTER`，红字 `#f31313`，副文案黑字 10px uppercase，右侧 28px 箭头。每位玩家每 12 小时最多制作 1 张海报；冷却期间按钮变为深绿 `#17664b` 并显示剩余时间。
...
- 详情底部是简洁社交信息区：顶部 1px 细线，显示 38px 圆形头像或首字母 fallback、9px 类型标签、20px 用户名、点赞数、留言数、点赞按钮、最近 3 条留言和 140 字留言输入框。用户名单行省略，连续长字符用 `overflow-wrap:anywhere` 折断。非本人作者和留言作者可点击打开 Aigram 主页。
