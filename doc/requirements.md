...
- 详情页是全屏海报展台，不是弹窗。顶部左侧为 32px 高透明细线 `Back` 按钮；主体区域展示 246px × 369px 完整海报，背景继续使用 `#151515`，只用 `0 26px 80px rgba(0,0,0,0.52)` 阴影托起海报。详情页不显示“正面/反面”说明文字。
...
- 演出海报 prompt 使用 9 个模板随机池，按“是否有头像 + 用户名 + 生成时间种子”选择 1 个模板，并在模板内再抽取 3 组色彩和 3 组字体策略。模板包括 `hex-zine-portrait`、`upstairs-handbill`、`tokyo-pulp-flyer`、`subway-showbill`、`swiss-type-system`、`color-block-program`、`fashion-week-roster`、`museum-comedy-label`、`xerox-photo-silhouette`。其中 `subway-showbill`、`swiss-type-system`、`color-block-program` 是更纯粹的平面设计模板，目前只用于有头像的 img2img 路径，因为无头像 txt2img 测试中高概率生成墙面 mockup；无头像仍使用用户名做主视觉，但暂时不抽高风险纯 ticket/grid 模板。
...
- 无头像演出海报 prompt：用户名成为主视觉身份来源，转成大面积裁切字母、首字母、票据编码、场馆戳、手写 stage name、vertical fragments 或半可读标题，而不是普通署名。UI 提示“Create an avatar to unlock a more personal poster identity.”
