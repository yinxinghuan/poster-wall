...
- 渲染方式是 DOM/CSS：英文单语场馆导视风海报展台、`STACK / GRID` 切换动画、生成等待页、完整海报详情页和评审页都由 HTML/CSS 组成。
...
- `src/PosterWall/PosterWall.tsx`：真实游戏 UI，包含英文单语文案、导视色块标题、`STACK / GRID` 视图状态、同一批海报的布局切换、12 小时冷却状态、全屏生成等待页、单张完整海报详情页、点赞、留言和作者 profile 入口。
- `src/PosterWall/PosterWall.less`：真实游戏的深黑背景、切角导视标牌、海报叠放布局、整齐排列布局、680ms 布局切换动画、色块生成入口、等待页、详情页和社交区样式。
...
- `public/img/review-posters/poster-00.svg` 到 `poster-11.svg`：离线评审和 demo 使用的 12 张英文切角导视风默认海报。
...
- 界面风格：`PosterWall.less` 使用深黑 `#151515`、奶白 `#eee7d5`、黄色 `#ffe100`、蓝色 `#2457e6`、红色 `#f31313`、深绿 `#17664b` 和青色 `#4dcfe8`。顶部 `.pw-header__copy` 是黄色切角导视牌，右侧 `.pw-view-toggle` 是并排的 `STACK / GRID` 模式牌；`STACK` 带三张重叠矩形图标，`GRID` 带 2×2 网格图标，选中态通过颜色和上移 5px 表达当前布局。底部 `.pw-generator` 保持克制，`.pw-cta` 是黄色切角 `MAKE POSTER` 标牌。
...
- 详情页：选中作品后渲染覆盖舞台的 `.pw-detail` 全屏状态，顶部红色切角 `BACK` 按钮关闭详情，点击海报本身也关闭。主体展示 246px × 369px 完整海报，不显示“正面/反面”说明；详情背景继续使用深黑，底部 `.pw-detail__social` 是细线分隔的信息区，包含作者头像 + 名字、点赞/留言计数、点赞按钮、最近 3 条留言和 140 字输入框；长用户名使用 `min-width:0`、`text-overflow: ellipsis`、`white-space: nowrap` 防止撑破页面。
...
- 换评审/默认样张：替换 `public/img/review-posters/poster-00.svg` 到 `poster-11.svg`；样张必须保持英文满版矩形，不要自带墙面、手机、边框或胶带。
